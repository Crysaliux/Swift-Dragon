...
class Swdconsole_logs:
    ...
    def error(self, code: str, time=None):
        if code == '100':
            self.raise_error(indexes.error + colors.ERROR + errors.unexpected_error)
        elif code == '200':
            self.raise_error(indexes.error + colors.ERROR + errors.user_not_found)
        elif code == '300':
            self.raise_error(indexes.error + colors.ERROR + errors.role_not_found)
        elif code == '400':
            self.raise_error(indexes.error + colors.ERROR + errors.internal_error)
        elif code == '500':
            self.raise_error(indexes.error + colors.ERROR + errors.timeout_error)
        elif code == '600':
            self.raise_error(indexes.error + colors.ERROR + errors.redirect_error)
        elif code == '700':
            self.raise_error(indexes.error + colors.ERROR + errors.blocked_prompt_error)
        elif code == '800':
            self.raise_error(indexes.error + colors.ERROR + errors.prompt_being_reviewed_error)
        elif code == '900':
            self.raise_error(indexes.error + colors.ERROR + errors.no_results_error)
        elif code == '1000':
            self.raise_error(indexes.error + colors.ERROR + errors.unsupported_lang_error)
        elif code == '1100':
            self.raise_error(indexes.error + colors.ERROR + errors.bad_images_error)
        elif code == '1200':
            self.raise_error(indexes.error + colors.ERROR + errors.no_images_error)
        elif code == '1300':
            self.raise_error(indexes.error + colors.ERROR + errors.server_not_found + f"At: {time}")
        elif code == '1400':
            self.raise_error(indexes.error + colors.ERROR + errors.token_not_found + f"At: {time}")
        else:
            self.raise_error(indexes.error + colors.ERROR + errors.internal_error)
    ...
    async def command_error(self, code: str, interaction, user=None, role=None):
        if code == '100':
    ...
    async def command_warning(self, code: str, interaction, user=None, role=None):
        if code == '010':
